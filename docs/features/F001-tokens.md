...
- **Core (no React, no next/*):** runtime theme state — init/get/set/toggle/onChange, extensible ThemeKey union, SSR-safe.
- **Stack A (Next.js 16/React 19/Tailwind v4/shadcn):** ThemeProvider (next-themes), @theme inline block in globals.css, ThemeToggle component; adapter re-exports headless store as a useTheme shim.
- **Stack B (Bun/Hono/Preact/Tailwind v4):** no next-themes — thin Preact useTheme hook over onThemeChange; identical CSS baseline; no next/* imports.
...
## Stories
- **F001.1** — Extract and publish headless theme-store package — _AC:_ @broberg/tokens exports init/get/set/toggle/onChange; SSR-safe; unit tests cover persistence, pub/sub, system-preference; published to pnpm workspace.
- **F001.2** — Ship CSS baseline template (neutral shadcn-compatible preset) — _AC:_ neutral-preset.css (from cms globals.css lines 1-214) declares :root dark-first tokens + 5 data-theme variants + @theme bridge; README shows the 3 lines to add to an app's globals.css.
- **F001.3** — Stack A React adapter (ThemeProvider + ThemeToggle) — _AC:_ ThemeProvider wraps layout, useTheme returns theme+setTheme, ThemeToggle renders mounted-guard Sun/Moon/Monitor dropdown; no hydration mismatch; works in Next.js 16 (pilot cms).
- **F001.4** — Stack B Preact adapter — _AC:_ useTheme Preact hook subscribes via onThemeChange; no next/* imports; works in a Bun/Hono/Preact app (pilot upmetrics or cardmem).
...
## Acceptance criteria
1. @broberg/tokens exports init/get/set/toggle/onChange, SSR-safe, unit-tested, published.
2. neutral-preset.css ships the :root tokens + 5 data-theme variants + @theme bridge; README documents adoption in 3 lines.
3. Stack A adapter: ThemeProvider + useTheme + ThemeToggle, no hydration mismatch, verified in Next.js 16.
4. Stack B adapter: Preact useTheme over onThemeChange, no next/* imports, verified in a Bun app.
5. cms adopts the baseline overriding only its gold primary; 5-variant system intact; Lens baseline pass.
6. Brand-override pattern documented with two worked examples.
7. @broberg/tokens builds + typechecks clean (tsc --noEmit), headless core imports no framework packages.
8. Piloted in cms and adopted back with no regression (Lens / runtime-verified, not just curl).
