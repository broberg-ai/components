...
## Solution
**hybrid.** The business logic (token gen, expiry, last-admin guard, role→permission resolution, duplicate-invite check, cross-org guard) is identical across cms (auth/invitations/team/permissions-shared), sanneandersen (invite route + db), trail (invite/auth), pitch (queries) — >=3 repos, identical → runtime-package. Persistence (JSON/SQLite-Drizzle/Supabase/bun:sqlite) and email transport differ per repo. The React invite-form + user-list UI is project-specific → copy-owned. So: headless core (package) + copy-owned UI shells = hybrid.
...
- **F009.2** — DrizzleUserStore adapter + pilot in cms — _AC:_ DrizzleUserStore<TSchema> implements both stores (any dialect); cms replaces inline fs auth/invitations with the package; all admin flows pass; JSON-file fallback store ships for cms file mode.
...
- **F009.5** — Copy-owned InviteForm scaffold (Stack A) — _AC:_ .claude/scaffolds InviteForm: CustomSelect role (no native <select>), email+name, POST to invite route, busy/error/success with visible feedback, data-testid on every interactive element, locale prop; matches sanneandersen.
- **F009.6** — Resend email adapter — _AC:_ sendInviteEmail (in package, not core) accepts {to, magicUrl, role, inviterName, locale}; dev returns devUrl; swap-in via the createInvitation callback.
