...
- **F012.3** — Stack B server route scaffold (Hono) — _AC:_ copy-owned profile.ts template documents 5 MB limit + image/* guard + putObject + avatar_key column; xrt81 profile.ts matches; README shows swapping storage backend.
...
- **F012.5** — Stack A server route scaffold (Next) — _AC:_ app/api/profile/avatar route POST+DELETE template; POST validates with the package, streams to Supabase/R2 by env, writes storage_path; DELETE nulls the column.
