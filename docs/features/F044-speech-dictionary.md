...
## Resolved: how the dictionary gets edited (2026-07-02, Christian)
Christian asked for an easy edit interface where saving either (a) auto-publishes a new npm version, or (b) the package becomes a thin client to a live API. (b) was ruled out — the primary use case (Trail's on-device WhisperKit correction) needs the dictionary bundled locally with zero network dependency; a live API would defeat exactly the offline/embeddable requirement cardmem's raw-JSON-files ask was for.

For (a), the first instinct was a web admin page (Discovery-hosted, auth-gated) — Christian explicitly rejected that: **"jeg orker ikke flere auth sider og user logins."** No new UI, no new auth surface. The actual answer needs nothing built: **editing is a chat instruction to any cc session** ("tilføj X → Y til ordbogen"), which edits `data/terms.json`/`data/corrections.json`, bumps the patch version, commits, and tag-pushes — the existing tag-triggered OIDC publish workflow (once Trusted Publisher is set up for this package, same as every other `@broberg/*` package) does the rest with no OTP and no login. No F044.2 story needed; there is no new surface to build.
