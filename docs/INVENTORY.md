...
- **Security / secret-redaction** → **`@broberg/secret-scan`** — *components-owned*, lifted from `broberg/trail` (F197): `redactSecrets(text)→{redacted,findings[]}` / `hasSecret(text)` / a curated ordered `SECRET_PATTERNS[]` (+ optional `extraPatterns`). Redacts secrets at comms in/out boundaries (ingest-gate + egress-scrub); pure, deterministic, dependency-free. **✅ shipped v0.1.0** (epic **F035**) — `@trail/shared` re-exports it and trail re-validated parity against the npm (0 leaks / 0 false positives). Christian to add OIDC trusted-publishing (`publish.yml`, tag `secret-scan-v*`) like `@broberg/theme`.
