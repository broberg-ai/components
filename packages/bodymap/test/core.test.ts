...
describe("REGIONS taxonomy", () => {
  it("has 15-20+ named regions with unique keys + codes", () => {
    ...
    expect(codes.size).toBe(REGIONS.length);
    for (const r of REGIONS) {
      expect(r.label).toBeTruthy();
      expect(r.code).toBeTruthy();
    }
  });
  it("getRegion looks up by key", () => {
    expect(getRegion("lower_back")?.code).toBe("LUMB");
...
describe("PainReport zod model", () => {
  ...
  it("accepts a valid pain point", () => {
    expect(painPointSchema.parse(base).region).toBe("lower_back");
  });
  it("rejects an unknown region", () => {
    expect(() => painPointSchema.parse({ ...base, region: "elbow_left" })).toThrow();
  });
...
describe("selection engine", () => {
  it("set/update/remove/getReport — one point per region, latest wins", () => {
    ...
    sel.set("knee_right", 7, "jagende"); // update same region
    sel.set("neck", 3);
...
describe("bodymap/v1 serialization", () => {
  it("serializes to { schema, view, points:[{region CODE, side, intensity, quality}] }", () => {
    const sel = createPainSelection([], { now });
    sel.set("lower_back", 8, "stikkende");
    sel.set("knee_right", 5, "dump");
    const env = serializeReport(sel.getReport(), { view: "front" });
    expect(env.schema).toBe("bodymap/v1");
    expect(env.view).toBe("front");
    expect(bodymapReportV1Schema.parse(env)).toEqual(env); // valid envelope
    const lumb = env.points.find((p) => p.region === "LUMB")!;
    expect(lumb).toMatchObject({ region: "LUMB", side: "center", intensity: 8, quality: "stikkende" });
    const knee = env.points.find((p) => p.region === "KNEE_R")!;
    expect(knee.side).toBe("right");
  });
  it("round-trips deserialize(serialize(report)) back to the same points", () => {
    const sel = createPainSelection([], { now });
    sel.set("neck", 6, "konstant");
    sel.set("foot_left", 2);
    const back = deserializeReport(serializeReport(sel.getReport()), now);
    expect(back.map((p) => p.region).sort()).toEqual(["foot_left", "neck"]);
    expect(back.find((p) => p.region === "neck")).toMatchObject({ intensity: 6, type: "konstant" });
  });
  it("drops points whose region CODE is unknown to this taxonomy", () => {
