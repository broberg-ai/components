...
export const DATA = [
 { n:"L0", t:"Rails", d:"foundation every app stands on", items:[
   ...
   {f:"F004",nm:"Config single-source helper",pkg:"@broberg/config",kw:["config","env","environment","dotenv","env vars","feature flags","settings","zod config","single source","validate env"],m:"runtime",e:"S",i:"high",s:"shipped",ver:"0.1.0",src:"broberg-ai/components",own:"components",desc:"The fleet's single-source config helper — enforces 'one source, trickle down' (Christian's no-hardcoded-values rule) as a reusable mechanism. parseEnv(schema, source?) validates + types process.env at boot via a Zod schema, failing fast with every offending key listed; defineConfig(obj) brands a typed business-constant object (fee tiers, magic numbers) as one import boundary; coerceInt/coerceBool are the no-Zod escape hatch (throw loudly on a malformed value, fall back when absent); productionGuard(config, keys) crashes the boot if a required secret is falsy in production. Zero runtime deps (zod is a peer); runs in Node/Bun/edge. Reference impl lifted from xrt81's env.ts + upmetrics' guard. Shipped as @broberg/config v0.1.0 — epic F004."},
   ...
   {f:"F006",nm:"Media / Cloudflare R2",pkg:"@broberg/media-r2",kw:["storage","object storage","file storage","r2","s3","upload","file upload","media upload","blob","bucket","assets"],m:"runtime",e:"M",i:"high",src:"broberg/cardmem",own:"cardmem",desc:"Object storage on Cloudflare R2 — upload, signed-URL and delete primitives for media, as a framework-agnostic runtime core."},
   ...
   {f:"F035",nm:"Secret / credential redaction",pkg:"@broberg/secret-scan",kw:["secret","secrets","redact","redaction","scrub","mask","credential","api key leak","pii","sanitize","sensitive data","secret scanning"],m:"runtime",e:"S",i:"high",s:"shipped",ver:"0.1.3",src:"broberg/trail",own:"cms",desc:"Pure, dependency-free secret/credential redaction — redactSecrets / hasSecret over a curated, ordered pattern set so an API key never lands in a database, a chat answer, or a shared knowledge base. Components-owned, lifted from trail F197; the canonical fleet pattern list. Shipped to npm as @broberg/secret-scan v0.1.3."},
 ...
 { n:"L1", t:"Identity", d:"who the user is", items:[
   ...
   {f:"F010",nm:"API-key + rate-limit",pkg:"@broberg/apikey",kw:["api key","apikey","rate limit","rate limiting","throttle","quota","access token","programmatic access"],m:"runtime",e:"M",i:"high",src:"broberg/trail",own:"trail",desc:"API-key issuance and rate-limiting — mint, store and throttle keys for programmatic access."},
...
export const FLEET = [
  { s:"components", r:"the shared-library home — this repo", pub:["theme","secret-scan","lens","seti-client","seti-server","mail","config"] },
