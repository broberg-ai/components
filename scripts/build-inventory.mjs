...
const DATA = [
 ...
 { n:"L3", t:"Domain", d:"feature surfaces", items:[
   ...
   {f:"seti-client",nm:"SETI streaming chat (client + Preact UI)",pkg:"@broberg/seti-client",m:"hybrid",e:"M",i:"high",s:"shipped",ver:"0.1.2",src:"broberg-ai/components",own:"components",desc:"The SETI streaming-chat client — a framework-agnostic core (FrameAccumulator scrollback engine + SetiClient: list / stream / sendText / sendKey over buddycloud's SETI API) plus a mobile-first Preact <SetiChat> component (status header, accumulated screen, nav-keys bar, delivery-feedback input; data-testid on every control). Lets any host app embed live cc-session streaming chat. Shipped as @broberg/seti-client v0.1.2 — epic F037 (contract = buddy F071.10; first consumer = cardmem's PLAN→Chat)."},
   {f:"seti-server",nm:"SETI proxy router",pkg:"@broberg/seti-server",m:"runtime",e:"S",i:"high",s:"shipped",ver:"0.1.0",src:"broberg-ai/components",own:"components",desc:"The SETI proxy router — a mountable Hono router (createSetiProxy) the host app mounts behind its OWN auth (app.route('/api/seti', …)). Pass-through to buddycloud's SETI API (GET /sessions, SSE /stream, POST /input); the consumer token stays server-side, so same-origin ⇒ no CORS and EventSource works with the host's cookie auth. Shipped as @broberg/seti-server v0.1.0 — epic F037."},
